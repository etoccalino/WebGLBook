...
MoveApp.prototype.changeToBlock = function () {
  ...
  {
    // Stop the animation
    this.model.animate && this.model.animate(false);
...
Robot.prototype.handleLoaded = function(data)
{
  if (data)
  {
	...
	// Walk through model looking for the "key" part
	var that = this;
	THREE.SceneUtils.traverseHierarchy(model, function (n) { that.traverseCallback(n); });

    // Setup the key animation and start it right away
...
Robot.prototype.traverseCallback = function (part)
{
  if (part.name == 'clef')
  {
    console.log('Key part found for the robot :)')
    this.key = part;
  }
}
...
Robot.prototype.createAnimation = function ()
{
  ...
  this.animator.init({
	...
	loop: true,
	duration:Robot.KEY_ANIMATION_TIME
  });

  this.addChild(this.animator);
}

Robot.prototype.handleMouseOver = function(x, y)
{
  this.animator.stop();
}

Robot.prototype.handleMouseOut = function(x, y)
{
  this.animator.start();
}

Robot.KEY_ANIMATION_TIME = 4000;
